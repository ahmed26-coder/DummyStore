                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <HeartIcon
                            className={`w-5 h-5 transition ${isFavorite(item.id)
                              ? "text-red-500"
                              : "text-black"
                              }`}
                          />
...
export function Home() {
  ...
  return (
    <div className="px-16 py-8" ref={sectionRef}>
...
export function CategoriesSection() {
        ...
        const productsData = await Promise.all(
          data.map(async (cat) => {
            try {
              ...
              return {
                name: cat,
                thumbnail: firstProduct?.thumbnail || "/placeholder.svg",
              }
            } catch {
              return { name: cat, thumbnail: "/placeholder.svg" }
            }
          })
        )
  ...
  return (
    <section className="container max-w-7xl mx-auto px-6 my-16">
      <h2 className="text-3xl font-bold text-[#3B82F6] mb-8 text-center">
        Shop by Category
      </h2>
      ...
      <Carousel className="w-full">
          ...
          {categories.map((cat, i) => (
            ...
            >
              <Card
                onClick={() =>
                  router.push(
                    `/product?category=${encodeURIComponent(cat.name)}&page=1`
                  )
                }
                className="group cursor-pointer py-0 rounded-2xl border shadow-sm hover:shadow-lg 
                     hover:border-[#3B82F6] transition-all duration-300 overflow-hidden"
              >
                <CardHeader className="p-0">
                  <div className="relative h-40 flex items-center justify-center bg-gradient-to-b from-gray-50 to-white">
                    <Image
                      src={cat.thumbnail}
                      alt={cat.name}
                      width={200}
                ...
                <CardContent className="flex flex-col items-center p-4">
                  <p className="capitalize font-semibold text-gray-800 mb-2 text-center line-clamp-1 group-hover:text-[#3B82F6]">
                    {cat.name.replace("-", " ")}
                  </p>
                  <button
                    className="flex items-center gap-1 text-sm font-medium text-[#3B82F6] opacity-0 
        ...
        {/* أزرار التحكم */}
        <CarouselPrevious className="left-2 sm:left-0 sm:-translate-x-14 flex" />
        <CarouselNext className="right-2 sm:right-0 sm:translate-x-14 flex" />
      </Carousel>
    </section>
  )
}
